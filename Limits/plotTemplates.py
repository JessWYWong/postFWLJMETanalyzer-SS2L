...
lumiInTemplates='41p53'

# sig='ttm800' # choose the 1st signal to plot
# sigleg='TT(0.8 TeV)'

sig='TTM1000'#'X53X53LHM1100'#'tttt'#'X53X53RHM1100'#'TTM1000' # choose the 1st signal to plot
sigleg='T#bar{T}(1 TeV)'#'(X_{5/3}#bar{X_{5/3}})_{L} (1.1TeV)'#'t#bar{t}t#bar{t}'#'X_{5/3}#bar{X_{5/3}} (1.1TeV) - RH'#'T#bar{T}(1 TeV)'

sigM1500='TTM1500'#'X53X53RHM1500'#'tttt' #'X53X53RHM1600'#'TTM1500' 
siglegM1500='T#bar{T}(1.5 TeV)'#'(X_{5/3}#bar{X_{5/3}})_{L} (1.5TeV)'#'t#bar{t}t#bar{t}' #'X_{5/3}#bar{X_{5/3}} (1.6TeV) - RH'#'T#bar{T}(1.5 TeV)'
...
systematicList = ['pileup','prefire','pdfNew20172018','muRFcorrdNew','jec2017','jer2017','Trig17','elIdSF2017','NPWeightEl2017','NPWeightMu2017','NPWeightElPR2017','NPWeightMuPR2017'] #'NPSys2017'
#systematicList = []
...
# 	if discriminant != 'ST': continue
	fileTemp='templates_'+sig[:sig.find('M')]+'_bW0p5_tZ0p25_tH0p25'+isRebinned+'.root'
        ...
        if 'tttt' in sig: fileTemp='templates_TTTTsig_'+isRebinned+'.root'
	...
	for isEM in isEMlist:
		...
		histPrefix=isEM+'2017BF' #iscriminant+'_'+lumiInTemplates+'fb_'+isEM
		
		try: hTOP = RFile.Get(histPrefix+topName).Clone()
		except: 
		...
		if doAllSys:
			for sys in systematicList:
				for ud in ['Up','Down']:
					try: systHists['top'+sys+ud] = RFile.Get(histPrefix+topName+'__'+sys.replace('muRFcorrdNew','muRFcorrdNewTTV').replace('Trig',TrigCat[isEM]+'Trig')+''+ud).Clone()
					except: 
					...
					try: systHists['qcd'+sys+ud] = RFile.Get(histPrefix+'__qcd__'+sys+''+ud).Clone()
						...
						pass
					try: 
						systHists['ddbkg'+sys+ud] = RFile.Get(histPrefix+'__FakeRate__'+sys+''+ud).Clone()
						print("Get ",histPrefix+'__FakeRate__'+sys+''+ud)
					except: 
		...
		try: hDDBKGstatOnly = hDDBKG.Clone(hDDBKG.GetName()+'statOnly')
                ...
                hDDBKGstatYr = {}
                hCMIDstatYr = {}
                try: hTOPstatYr['17'] = RFile.Get(histPrefix.replace('1718','2017BF')+topName).Clone(hTOP.GetName()+'statOnly17')
                except: pass
                ...
                try: hTOPstatYr['18'] = RFile.Get(histPrefix.replace('1718','2018AD')+topName).Clone(hTOP.GetName()+'statOnly18')
                except: pass
			...
			if doAllSys:
				for sys in systematicList:
					...
					if 'PR' in sys or 'FR' in sys or 'NP' in sys:
						...
						try: 
							if not sys=="NPWeightMu2017":errorPlus = systHists['ddbkg'+sys+'Up'].GetBinContent(ibin)-hDDBKG.GetBinContent(ibin)
							else: errorPlus = 0 # one-sided NPweights, -0.02 for Mu
						except: pass
						try: 
							if not sys=="NPWeightEl2017":errorMinus = hDDBKG.GetBinContent(ibin)-systHists['ddbkg'+sys+'Down'].GetBinContent(ibin)
						...
						if errorMinus > 0: errorDn += errorMinus**2
                                                ...
                                                if('ddbkg'+sys+'Up' in systHists.keys()): print("NPerror", errorPlus, errorMinus, systHists['ddbkg'+sys+'Up'].GetName(), ibin, hDDBKG.GetBinContent(ibin))
...
# 		if not blind: hData.Draw("E1 X0")
		...
		gaeData.SetMaximum(1.5*max(gaeData.GetMaximum(),bkgHT.GetMaximum()))
		...
		hsig.Draw("SAME HIST")
		if not sigM1500=='tttt': hsigM1500.Draw("SAME HIST")
...
# 		try: leg.AddEntry(hDDBKG,"DD BKG","f")
		...
		leg.AddEntry(hsig,sigleg+scaleFact1Str,"l")
                ...
                if not sigM1500=='tttt': leg.AddEntry(hsigM1500,siglegM1500+scaleFact2Str,"l")
...
#			pull.Divide(pullDenom)
			...
			#for binNo in range(hData.GetNbinsX()):
			#	if bkgHT.GetBinContent(binNo)!=0:
			#		print("data/bkg bin "+ str(binNo) +" = "+str(hData.GetBinContent(binNo))+"/"+str(bkgHT.GetBinContent(binNo))+" = "+str(hData.GetBinContent(binNo)/bkgHT.GetBinContent(binNo))+" v.s. "+str(pull.GetBinContent(binNo)))
