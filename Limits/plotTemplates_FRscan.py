...
#systematicList = ['pileup','prefire','pdfNew20172018','muRFcorrdNew','jec2017','jer2017','Trig17','elIdSF2017','NPSys2017','NPWeightEl2017','NPWeightMu2017']
systematicList = ['pileup','prefire','pdfNew20172018','muRFcorrdNew','jec2017','jer2017','Trig17','elIdSF2017']
...
muFRind = -1
if len(sys.argv)>4: 
  elFRind = int(sys.argv[4])
  try: muFRind = int(sys.argv[5])
  except: 
    print("must input both elFRind and muFRind as 5th and 6th argument")
    exit(1)
...
# 	if discriminant != 'ST': continue
	fileTemp='templates_'+sig[:sig.find('M')]+'_bW0p5_tZ0p25_tH0p25'+isRebinned+'.root'
        if discriminant != 'cleanAK4HT': fileTemp=fileTemp.replace('templates_', 'templates_'+discriminant+'_')
	...
	for isEM in isEMlist:
		...
		setTDRStyle()
                FRind = 10
		if isEM=="elel" and elFRind >= 0: FRind = elFRind
		if isEM=="mumu" and muFRind>=0: FRind = muFRind
		...
		try: hQCD = RFile.Get(histPrefix+'__qcd').Clone()
			...
			pass
                print(histPrefix+'__FakeRate__NPWeight2017'+str(FRind))
		try: hDDBKG = RFile.Get(histPrefix+'__FakeRate__NPWeight2017'+str(FRind)).Clone()
		except:
		...
		try: hDDBKGstatOnly = hDDBKG.Clone(hDDBKG.GetName()+'statOnly')
                ...
                try: hDDBKGstatYr['17'] = RFile.Get(histPrefix.replace('1718','2017BF')+'__FakeRate__NPWeight2017'+str(FRind)).Clone(hDDBKG.GetName()+'statOnly17')
                except: pass
                ...
                try: hDDBKGstatYr['18'] = RFile.Get(histPrefix.replace('1718','2018AD')+'__FakeRate__NPWeight2017'+str(FRind)).Clone(hDDBKG.GetName()+'statOnly18')
                except: pass
			...
			if doAllSys:
				for sys in systematicList:
					...
					if 'PR' in sys or 'FR' in sys or 'NP' in sys:
						...
						try: errorPlus = systHists['ddbkg'+sys+'Up'].GetBinContent(ibin)-hDDBKG.GetBinContent(ibin)
						except: pass
						try: errorMinus = hDDBKG.GetBinContent(ibin)-systHists['ddbkg'+sys+'Down'].GetBinContent(ibin)
                                                except: pass
                                                if errorPlus > 0: errorUp += errorPlus**2
                                                else: errorDn += errorPlus**2
                                                if errorMinus > 0: errorDn += errorMinus**2
						...
						if errorMinus > 0: errorDn += errorMinus**2
                                                ...
                                                if('ddbkg'+sys+'Up' in systHists.keys()): print("NPerror", errorPlus, errorMinus, systHists['ddbkg'+sys+'Up'].GetName())
...
#			else: pullLegend.AddEntry(pullUncBandTot , "Bkg stat. #oplus syst." , "f")
		...
		savePrefix = templateDir+'/plots_FRscan/'
