...
int main(int argc, char* argv[]){
  ...
  for (int mass = mass_lowEdge; mass < mass_upEdge+100; mass+=100){
    if(mass!= 1000 && mass!= 1500) continue; // ******JESS : only 1000 adn 1500 samples for njet3!!!!!******
  ...
  for(std::vector<Sample*>::size_type i =0; i< vBkg.size(); i++){
    ...
    if(doNPSys && vBkg.at(i)->name=="NonPrompt"){
      ...
      for(int iBin = 1; iBin< elel_bkg->GetNbinsX()+1; iBin++){
        if(elel_bkg->GetXaxis()->GetBinUpEdge(iBin)<=400){eeFRunc = 0.194; emFRunc = 0.431; mmFRunc = 0.569;}
        else if(elel_bkg->GetXaxis()->GetBinUpEdge(iBin)<=600){eeFRunc = 0.139; emFRunc = 0.302; mmFRunc = 0.544;}
        else{eeFRunc = 0.461; emFRunc = 0.168; mmFRunc = 0.151;}
