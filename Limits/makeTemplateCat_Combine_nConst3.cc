...
int main(int argc, char* argv[]){
  ...
  //double xbins[12] = {500, 600, 700, 800, 900, 1000, 1100, 1200, 1300, 1400, 1600, 3000};
  //double xbins[5] = {1200, 1300, 1400, 1600, 3000};
  ...
  //double xbins[15] = {200, 300.0, 400.0, 500.0, 600.0, 700.0, 800.0, 900.0, 1000.0, 1100.0, 1200.0, 1300.0, 1500.0, 1700.0, 3000.0};
  //double xbins[13] = {200, 300, 400, 500, 600, 700, 800, 900, 1000, 1200, 1400, 1600, 3000};
  //double xbins[15] = {200, 300, 400, 500, 600, 700, 800, 900, 1000, 1100, 1200, 1300, 1500, 1700, 3000};
  double xbins[13] = {200, 300, 400, 500, 600, 700, 800, 900, 1000, 1100, 1300, 1500, 3000};
  ...
  std::vector<std::string> systList {"IDSF","trigSF","PUWeight"};
  std::vector<TString> systNameList {"elIdSF2018","Trig18","pileup"};
  ...
  if(isCR && !(xbins[nxbins]==maxHT)){std::cout<<"WARNING! HT upper cut not matching the defined bins up edge "<< xbins[nxbins-1] <<std::endl; return 1;}
  ...
  for (int mass = mass_lowEdge; mass < mass_upEdge+100; mass+=100){
    ...
    //if(mass!= 1000 && mass!= 1500) continue;
	...
	for(std::vector<Sample*>::size_type i=0; i< sigSample.size(); i++){
      ...
      if(name=="TTM900")  {scaleSFsUp = 0.744; scaleSFsDn = 1.312;}
      if(name=="TTM1000") {scaleSFsUp = 0.743; scaleSFsDn = 1.314;}
      if(name=="TTM1100") {scaleSFsUp = 0.737; scaleSFsDn = 1.323;}
      if(name=="TTM1200") {scaleSFsUp = 0.740; scaleSFsDn = 1.318;}
      if(name=="TTM1300") {scaleSFsUp = 0.741; scaleSFsDn = 1.318;}
      if(name=="TTM1400") {scaleSFsUp = 0.737; scaleSFsDn = 1.324;}
      if(name=="TTM1500") {scaleSFsUp = 0.737; scaleSFsDn = 1.324;}
      if(name=="TTM1600") {scaleSFsUp = 0.734; scaleSFsDn = 1.328;}
      if(name=="TTM1700") {scaleSFsUp = 0.735; scaleSFsDn = 1.330;}
      if(name=="TTM1800") {scaleSFsUp = 0.740; scaleSFsDn = 1.321;}
      if(name=="BBM900")  {scaleSFsUp = 0.744; scaleSFsDn = 1.312;}
      if(name=="BBM1000") {scaleSFsUp = 0.742; scaleSFsDn = 1.315;}
      if(name=="BBM1100") {scaleSFsUp = 0.742; scaleSFsDn = 1.316;}
      if(name=="BBM1200") {scaleSFsUp = 0.741; scaleSFsDn = 1.318;}
      if(name=="BBM1300") {scaleSFsUp = 0.741; scaleSFsDn = 1.317;}
      if(name=="BBM1400") {scaleSFsUp = 0.739; scaleSFsDn = 1.321;}
      if(name=="BBM1500") {scaleSFsUp = 0.735; scaleSFsDn = 1.328;}
      if(name=="BBM1600") {scaleSFsUp = 0.735; scaleSFsDn = 1.329;}
      if(name=="BBM1700") {scaleSFsUp = 0.720; scaleSFsDn = 1.356;}
      if(name=="BBM1800") {scaleSFsUp = 0.855; scaleSFsDn = 1.272;}
      ...
      if(name=="TTM1100") {pdfSFsUp = 0.951; pdfSFsDn = 1.055;}
      if(name=="TTM1200") {pdfSFsUp = 0.947; pdfSFsDn = 1.060;}
      if(name=="TTM1300") {pdfSFsUp = 0.942; pdfSFsDn = 1.065;}
      ...
      if(name=="BBM1200") {pdfSFsUp = 0.947; pdfSFsDn = 1.060;}
      if(name=="BBM1300") {pdfSFsUp = 0.942; pdfSFsDn = 1.066;}
      if(name=="BBM1400") {pdfSFsUp = 0.937; pdfSFsDn = 1.073;}
      if(name=="BBM1500") {pdfSFsUp = 0.929; pdfSFsDn = 1.082;}
      if(name=="BBM1600") {pdfSFsUp = 0.921; pdfSFsDn = 1.094;}
      if(name=="BBM1700") {pdfSFsUp = 0.911; pdfSFsDn = 1.109;}
      if(name=="BBM1800") {pdfSFsUp = 0.898; pdfSFsDn = 1.128;}
  ...
  for(std::vector<Sample*>::size_type i =0; i< vBkg.size(); i++){
    ...
    if(doNPSys && vBkg.at(i)->name=="NonPrompt"){
      TString postfix_NP = postfix+"__NPSys2018";
      ...
      for(int iBin = 1; iBin< elel_bkg->GetNbinsX()+1; iBin++){
        if(elel_bkg->GetXaxis()->GetBinUpEdge(iBin)<=400){eeFRunc = 0.156; emFRunc = 0.375; mmFRunc = 0.526;}
        else if(elel_bkg->GetXaxis()->GetBinUpEdge(iBin)<=600){eeFRunc = 0.320; emFRunc = 0.210; mmFRunc = 0.341;}
        else{eeFRunc = 0.106; emFRunc = 0.138; mmFRunc = 0.207;}
      ...
      TString postfix_uElFR = postfix+"__NPWeightEl2018";
      ...
      writeHist.push_back((TH1F*)getPlot(vBkg.at(i),"elel"+postfix_uElFR+"Down","cleanAK4HT",nxbins,xbins,cutSStream.str(),0,true,"(NPWeight/NPWeight) * ")); //FRscan + uelFR
      writeHist.push_back((TH1F*)getPlot(vBkg.at(i),"elmu"+postfix_uElFR+"Down","cleanAK4HT",nxbins,xbins,cutSStream.str(),1,true,"(NPWeight/NPWeight) * ")); //FRscan + uelFR
      TString postfix_uMuFR = postfix+"__NPWeightMu2018";
      writeHist.push_back((TH1F*)getPlot(vBkg.at(i),"elmu"+postfix_uMuFR+"Up","cleanAK4HT",nxbins,xbins,cutSStream.str(),1,true,"(NPWeight/NPWeight) * ")); //FRscan - umuFR
      writeHist.push_back((TH1F*)getPlot(vBkg.at(i),"mumu"+postfix_uMuFR+"Up","cleanAK4HT",nxbins,xbins,cutSStream.str(),2,true,"(NPWeight/NPWeight) * ")); //FRscan - umuFR
      ...
      TString postfix_uElPR = postfix+"__NPWeightElPR2018";
      ...
      TString postfix_uMuPR = postfix+"__NPWeightMuPR2018";
    ...
    if(isCR && doFRScan && vBkg.at(i)->name=="NonPrompt"){
      TString postfix_FRscan = postfix+"__NPWeight2018";
  ...
  for(unsigned int h =0; h<writeHist.size();h++){
    ...
    for(auto it=vGrpBkg.begin(); it!=vGrpBkg.end(); ++it){
      for(auto sample : it->second){
        if(sample==(std::string)bkg) {grpHist[(std::string)(chn+"__"+it->first+type)]->Add(writeHist.at(h)); std::cout<< hname<<" added to "<< it->first <<std::endl;}
      }
    }
  }
