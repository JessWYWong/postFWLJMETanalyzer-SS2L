...
//   std::vector<Sample*> vSig = getInclusiveSigSampleVecForTable("sZVeto",lumi,elID,muID,era);
  ...
  if(whichSignal == "TT"){
    ...
    std::cout<< "vSigSamples1 size "<<vSigSamples1.size()<<std::endl;
  ...
  //std::cout<<"vSig.size() = "<<vSig.size()<<std::endl;
  ...
  //std::cout<<"vSig.size() = "<<vSig.size()<<std::endl;
  //std::cout<<"mass "<<mass_lowEdge << mass_upEdge<<std::endl;
  ...
  for (int mass = mass_lowEdge; mass < mass_upEdge+100; mass+=100){
    ...
    for(std::vector<Sample*>::size_type i=0; i< vSig.size(); i++){
      ...
      if( vSig.at(i)->name.find(mstring)!=std::string::npos){
        ...
        std::cout<< "include "<<vSig.at(i)<<std::endl;
        //break;
      }
    }
