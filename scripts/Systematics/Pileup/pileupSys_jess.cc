...
//   TString puDowncut = "PUWeightDown * MCWeight * (Lep1Pt>40 && nConst >=5 && (DilepMass>20) && ( (Channel!=0) ||(DilepMass<76.1 || DilepMass >106.1) ) && cleanAK4HT > 1200)";
  TString baseWeight = "ChargeMisIDWeight * NPWeight * IDSF * IsoSF * trigSF * GsfSF * prefireWeight * ";
  //TString baseWeight = "IDSF * IsoSF * trigSF * GsfSF * prefireWeight * ";
  //TString baseWeight = "";

  TString nomcut =    baseWeight+"PUWeight * MCWeight * (Lep1Pt>40 && nConst >=4 && (DilepMass>20) && ( (Channel!=0) ||(DilepMass<76.1 || DilepMass >106.1) ) && cleanAK4HT > 1200)";
  TString puUpcut =   baseWeight+"PUWeightUp * MCWeight * (Lep1Pt>40 && nConst >=4 && (DilepMass>20) && ( (Channel!=0) ||(DilepMass<76.1 || DilepMass >106.1) ) && cleanAK4HT> 1200)";
  TString puDowncut = baseWeight+"PUWeightDown * MCWeight * (Lep1Pt>40 && nConst >=4 && (DilepMass>20) && ( (Channel!=0) ||(DilepMass<76.1 || DilepMass >106.1) ) && cleanAK4HT > 1200)";
*/

  TString nomcut = "ChargeMisIDWeight * NPWeight * IsoSF * trigSF * GsfSF * prefireWeight * PUWeight * IDSF * MCWeight * (Lep1Pt>40 && nConst >=4 && (DilepMass>20) && ( (Channel!=0) ||(DilepMass<76.1 || DilepMass >106.1) ) && cleanAK4HT > 1200)";
  TRegexp re("PUWeight");
  TString puUpcut = nomcut; puUpcut(re) = "PUWeightUp";
  TString puDowncut = nomcut; puDowncut(re) = "PUWeightDown";
...
//   }
  ...
  filecontent<<"uncDict['puUp']['"<< sample << "']="<<1+puupPct<< "\n";
  filecontent<<"uncDict['puDn']['"<< sample << "']="<<1+pudownPct<< "\n";
...
void printLine(std::string sample, TString filename){
  ...
  float sysUp = 1+puupPct/100.;
  float sysDn = 1+pudownPct/100.;
...
void pileupSys_jess(){
  ...
  */
  ...
  for( it = uncSig.begin(); it != uncSig.end(); it++ ){
    ...
    filecontent_sig<<"uncDict['puUp']['"<< it->first << "']="<< pow((it->second)[0]/(countTT),0.5) << "\n";
    filecontent_sig<<"uncDict['puDn']['"<< it->first << "']="<< pow((it->second)[1]/(countTT),0.5) << "\n";
...
//   TString MCfolder ="/uscms_data/d3/rsyarif/CERN2017/produceLjmetNtuples_TTSSdilep/CMSSW_8_0_26_patch1/src/PostLJMet/X53ThirteenTeVAnalysisCode/test/ProcessedByJulie_Oct18-2017/test/";
  ...
  outfile.open("unc_pileup_MCBkg.py");

  std::stringstream filecontent;
  filecontent<<std::fixed<<std::setprecision(5);
  
  filecontent<< "uncDict = {} \n";
  filecontent<< "uncDict['puUp'] = {} \n";
  filecontent<< "uncDict['puDn'] = {} \n";
