...
/*
  TString baseWeight = "ChargeMisIDWeight * NPWeight * IDSF * IsoSF * trigSF * GsfSF * PUWeight * ";
  //TString baseWeight = "IDSF * IsoSF * trigSF * GsfSF * prefireWeight * ";
  //TString baseWeight = "";

  TString nomcut =    baseWeight+"prefireWeight * MCWeight * (Lep1Pt>40 && nConst >=4 && (DilepMass>20) && ( (Channel!=0) ||(DilepMass<76.1 || DilepMass >106.1) ) && cleanAK4HT > 1200)";
  TString Upcut =   baseWeight+"prefireWeightUp * MCWeight * (Lep1Pt>40 && nConst >=4 && (DilepMass>20) && ( (Channel!=0) ||(DilepMass<76.1 || DilepMass >106.1) ) && cleanAK4HT> 1200)";
  TString Downcut = baseWeight+"prefireWeightDown * MCWeight * (Lep1Pt>40 && nConst >=4 && (DilepMass>20) && ( (Channel!=0) ||(DilepMass<76.1 || DilepMass >106.1) ) && cleanAK4HT > 1200)";
*/
  ...
  TRegexp re("prefireWeight");
  TString Upcut = nomcut; Upcut(re) = "prefireWeightUp";
  TString Downcut = nomcut; Downcut(re) = "prefireWeightDown";
...
//   }
  ...
  filecontent<<"uncDict['prefireUp']['"<< sample << "']="<<1+upPct<< "\n";
  filecontent<<"uncDict['prefireDn']['"<< sample << "']="<<1+downPct<< "\n";
...
void printLine(std::string sample, TString filename){
  ...
  TH1D* hup = (TH1D*)f->Get("hist_prefireup");
  float up = hup->Integral();
  float upPct = 100*(up-nom)/nom;

  TH1D* hdown = (TH1D*)f->Get("hist_prefiredown");
...
void prefireSys_jess(){
  ...
  for(int BRtype=0;BRtype<23;BRtype++){
	  ...
	  //print top line
	  std::cout<<" Sample | prefire Up | prefire Down"<<std::endl;
  ...
  */
  ...
  outfile_sig.open("unc_prefire_sig.py");

  std::stringstream filecontent_sig;
  filecontent_sig<<std::fixed<<std::setprecision(5);

  filecontent_sig<< "uncDict = {} \n";
  filecontent_sig<< "uncDict['prefireUp'] = {} \n";
  filecontent_sig<< "uncDict['prefireDn'] = {} \n";
  ...
  for( it = uncSig.begin(); it != uncSig.end(); it++ ){
    ...
    filecontent_sig<<"uncDict['prefireUp']['"<< it->first << "']="<< pow((it->second)[0]/(countTT),0.5) << "\n";
    filecontent_sig<<"uncDict['prefireDn']['"<< it->first << "']="<< pow((it->second)[1]/(countTT),0.5) << "\n";
...
//   TString MCfolder ="/uscms_data/d3/rsyarif/CERN2017/produceLjmetNtuples_TTSSdilep/CMSSW_8_0_26_patch1/src/PostLJMet/X53ThirteenTeVAnalysisCode/test/ProcessedByJulie_Oct18-2017/test/";
  ...
  filecontent<< "uncDict = {} \n"; 
  filecontent<< "uncDict['prefireUp'] = {} \n";
  filecontent<< "uncDict['prefireDn'] = {} \n";
