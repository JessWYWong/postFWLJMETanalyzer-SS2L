...
/*
  TString baseWeight = "ChargeMisIDWeight * NPWeight * IsoSF * trigSF * GsfSF * prefireWeight * PUWeight * ";
  //TString baseWeight = "IDSF * IsoSF * trigSF * GsfSF * prefireWeight * ";
  //TString baseWeight = "";

  TString nomcut =    baseWeight+"IDSF * MCWeight * (Lep1Pt>40 && nConst >=4 && (DilepMass>20) && ( (Channel!=0) ||(DilepMass<76.1 || DilepMass >106.1) ) && cleanAK4HT > 1200)";
...
//   }
  ...
  filecontent<<"uncDict['idSFUp']['"<< sample << "']="<<1+upPct<< "\n";
  filecontent<<"uncDict['idSFDn']['"<< sample << "']="<<1+downPct<< "\n";
...
void initSigMap(){
  ...
  /*{
    //SUMSQUp_SUMSQDn_maxUp_maxDn_minUp_minDn
    {"TTM1000",{0.,0.,0.,0.,0.,0.}}, {"TTM1100",{0.,0.,0.,0.,0.,0.}}, {"TTM1200",{0.,0.,0.,0.,0.,0.}}, {"TTM1300",{0.,0.,0.,0.,0.,0.}},
    {"TTM1400",{0.,0.,0.,0.,0.,0.}}, {"TTM1500",{0.,0.,0.,0.,0.,0.}}, {"TTM1600",{0.,0.,0.,0.,0.,0.}}, {"TTM1700",{0.,0.,0.,0.,0.,0.}},
    {"TTM1800",{0.,0.,0.,0.,0.,0.}}, {"BBM900",{0.,0.,0.,0.,0.,0.}} , {"BBM1000",{0.,0.,0.,0.,0.,0.}}, {"BBM1100",{0.,0.,0.,0.,0.,0.}},
    {"BBM1200",{0.,0.,0.,0.,0.,0.}}, {"BBM1300",{0.,0.,0.,0.,0.,0.}}, {"BBM1400",{0.,0.,0.,0.,0.,0.}}, {"BBM1500",{0.,0.,0.,0.,0.,0.}},
    {"BBM1600",{0.,0.,0.,0.,0.,0.}}, {"BBM1700",{0.,0.,0.,0.,0.,0.}}, {"BBM1800",{0.,0.,0.,0.,0.,0.}}
  };*/
}
...
void printLine(std::string sample, TString filename){
  ...
  TH1D* hup = (TH1D*)f->Get("hist_idup");
  float up = hup->Integral();
  float upPct = 100*(up-nom)/nom;

  TH1D* hdown = (TH1D*)f->Get("hist_iddown");
...
void idSFSys_jess(){
  ...
  for(int BRtype=0;BRtype<23;BRtype++){
	  ...
	  //print top line
	  std::cout<<" Sample | idSF Up | idSF Down"<<std::endl;
  ...
  for( it = uncSig.begin(); it != uncSig.end(); it++ ){
    ...
    filecontent_sig<<"uncDict['idSFUp']['"<< it->first << "']="<< pow((it->second)[0]/(countTT),0.5) << "\n";
    filecontent_sig<<"uncDict['idSFDn']['"<< it->first << "']="<< pow((it->second)[1]/(countTT),0.5) << "\n";
...
//   TString MCfolder ="/uscms_data/d3/rsyarif/CERN2017/produceLjmetNtuples_TTSSdilep/CMSSW_8_0_26_patch1/src/PostLJMet/X53ThirteenTeVAnalysisCode/test/ProcessedByJulie_Oct18-2017/test/";
  ...
  filecontent<< "uncDict = {} \n";
  filecontent<< "uncDict['idSFUp'] = {} \n";
  filecontent<< "uncDict['idSFDn'] = {} \n";
