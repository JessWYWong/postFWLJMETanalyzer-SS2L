...
int main(int argc, char* argv[]){
  ...
  if(argv1=="Data") {
    ...
    // Input folder
    //std::string filedir = "FWLJMET102X_2lep2018_wywong_052020_hadds";
    //std::string eosDir = "user/lpcljm/";
    std::string filedir = "FWLJMET102X_2lep2018_wywong_082020_hadds";
    std::string eosDir = "user/lpcljm/";
    // Input files
    if(MuonChannel){
      if(argv4=="2018A") filename="root://cmseos.fnal.gov//store/"+eosDir+filedir+"/DoubleMuonRun2018A.root";
      else if(argv4=="2018B") filename="root://cmseos.fnal.gov//store/"+eosDir+filedir+"/DoubleMuonRun2018B.root";
      else if(argv4=="2018C") filename="root://cmseos.fnal.gov//store/"+eosDir+filedir+"/DoubleMuonRun2018C.root";
      else if(argv4=="2018D") filename="root://cmseos.fnal.gov//store/"+eosDir+filedir+"/DoubleMuonRun2018D.root";
    }
    else{            
  ...
  for(int ient=0; ient<nEntries; ient++){
    ...
    //else{
    // always require passing dilepton triggers
      if(MuonChannel){
        if(isoTrig){
          //if(tr->HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ_Mass3p8) passTrig=true;
          if(tr->HLT_Mu17_TrkIsoVVL_Mu8_TrkIsoVVL_DZ_Mass8_v) passTrig=true;
        }
        else{
