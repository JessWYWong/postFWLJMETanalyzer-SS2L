...
def create_condor_files_from_template(rel_base,cmssw,locdir,eosdir,samples,rootfile_paths,submit=False):
	...
	for sample in samples:
		...
		condor_file.close()
                ...
                shcondor_templ_file = open(rel_base+"/src/postFWLJMETanalyzer/postLJMEThadd/PostLJMetHadd_shcondor.templ")
                localshcondor = locdir+'/'+sample+"_sh.condor"
                shcondor_file = open(localshcondor,"w")
                for line in shcondor_templ_file:
                        line=line.replace('DIRECTORY',locdir)
                        line=line.replace('SAMPLENAME',sample)
                        shcondor_file.write(line)
                shcondor_file.close()
		...
		csh_file.close()
                ...
                sh_templ_file    = open(rel_base+"/src/postFWLJMETanalyzer/postLJMEThadd/PostLJMetHadd_sh.templ") 
                localsh=locdir+'/'+sample+".sh"
